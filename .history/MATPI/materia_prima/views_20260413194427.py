...
def importar_materia_prima_excel(request):
    ...
    if request.method == 'POST' and request.FILES.get('archivo_excel'):
        ...
        try:
            ...
            # 1. Verificar si hay duplicados antes de importar nada
            for row in rows:
            ...
            # 2. Si no hay duplicados, importar
            creados = 0
            with transaction.atomic():
                for row in rows:
                    nombre, unidad, cant_unidad, tipo = row[:4]
...
def importar_lotes_excel(request):
    ...
    if request.method == 'POST' and request.FILES.get('archivo_excel'):
        ...
        try:
            ...
            # 1. Validación previa de existencia de materias y duplicados de lotes
            for row_idx, row in enumerate(rows, start=2):
                ...
                # Buscar materia prima (misma lógica que antes)
